...
#game is the main class of the program, it is the one which controls the display and uses the other classes as ressources
...
#so we can now define the resolution of our game, scale being the pixel height and lenght of every case of the maze
        self.scale = min(max(listsize)//16, 50)

#loading every image and creating two rect objects to place the images, note that only Macgyver will have a rect object which will move while all other object will always be placed compare to the origin and therefore uses the same rect : originrect
        ...
        self.victory = pygame.transform.scale(pygame.image.load("victory.png"),(self.scale*16,self.scale*15))
        self.defeat = pygame.transform.scale(pygame.image.load("defeat.png"),(self.scale*16,self.scale*15))
...
#distinguishing the last column from the rest with a white background so the user doesn't mistake the inventory for the maze
...
#moving mac and updating the display accordingly as long as Mac can effectively move this way
...
#pickup is the action mac will always do when he enters a case, therefore it is not only about looting objects, but also about fighting the guardian
#first we verify wether there is a object to loot
        if self.map.map[mac.i][mac.j] == "2" and self.map.items[(mac.i,mac.j)] not in mac.items:
            ...
            pygame.display.flip()
#if there were one to loot, we check wether he can craft his syringue already
            if len(mac.items) >= 3:
                self.screen.blit(self.whiteimg, self.originrect.move(15*self.scale, self.scale))
...
#then we check wether he may fight the guardian
            ...
            pygame.display.flip()
#once the player either won or lost, we wait for him to close the window
            while 1:
